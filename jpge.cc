...
// been 1. Either way, it wasn't helping.)
// v1.03, Apr. 16, 2011 - Added support for optimized Huffman code tables,
...
//                       (all looked harmless).
//                       Code review revealed method load_block_16_8_8() (used
//                       for the non-default H2V1 sampling mode to downsample
//                       chroma) somehow didn't get the rounding factor fix from
//                       v1.02.
...
void jpeg_encoder::optimize_huffman_table(int table_num, int table_len) {
  ...
  const uint JPGE_CODE_SIZE_LIMIT = 16; // the maximum possible size of a JPEG
                                        // Huffman code (valid range is [9,16] -
                                        // 9 vs. 8 because of the dummy symbol)
...
void jpeg_encoder::load_block_16_8(int x, int c) {
  ...
  for (int i = 0; i < 16; i += 2, pDst += 8) {
    ...
    pDst[0] =
        ((pSrc1[0 * 3] + pSrc1[1 * 3] + pSrc2[0 * 3] + pSrc2[1 * 3] + a) >> 2) -
        128;
    pDst[1] =
        ((pSrc1[2 * 3] + pSrc1[3 * 3] + pSrc2[2 * 3] + pSrc2[3 * 3] + b) >> 2) -
        128;
    pDst[2] =
        ((pSrc1[4 * 3] + pSrc1[5 * 3] + pSrc2[4 * 3] + pSrc2[5 * 3] + a) >> 2) -
        128;
    pDst[3] =
        ((pSrc1[6 * 3] + pSrc1[7 * 3] + pSrc2[6 * 3] + pSrc2[7 * 3] + b) >> 2) -
        128;
    pDst[4] =
        ((pSrc1[8 * 3] + pSrc1[9 * 3] + pSrc2[8 * 3] + pSrc2[9 * 3] + a) >> 2) -
        128;
    pDst[5] =
        ((pSrc1[10 * 3] + pSrc1[11 * 3] + pSrc2[10 * 3] + pSrc2[11 * 3] + b) >>
         2) -
        128;
    pDst[6] =
        ((pSrc1[12 * 3] + pSrc1[13 * 3] + pSrc2[12 * 3] + pSrc2[13 * 3] + a) >>
         2) -
        128;
    pDst[7] =
        ((pSrc1[14 * 3] + pSrc1[15 * 3] + pSrc2[14 * 3] + pSrc2[15 * 3] + b) >>
         2) -
        128;
    int temp = a;
    a = b;
...
void jpeg_encoder::load_mcu(const void *pSrc) {
  ...
  uint8 *pDst = m_mcu_lines
      [m_mcu_y_ofs]; // OK to write up to m_image_bpl_xlt bytes to pDst
