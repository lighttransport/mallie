...
#define SC_(a) static_cast<float>(a)
		...
		vector3t(value_type _x, value_type _y, value_type _z):m0(_x),m1(_y),m2(_z){}
		
		vector3t(const this_type &rhs):m0(rhs.m0),m1(rhs.m1),m2(rhs.m2){}
		
		explicit vector3t(const value_type rhs[c_size]):m0(SC_(rhs[0])),m1(SC_(rhs[1])),m2(SC_(rhs[2])){}
		
		template<class X>
		vector3t(const vector3t<X>& rhs){
			m0 = SC_(rhs[0]);
			m1 = SC_(rhs[1]);
			m2 = SC_(rhs[2]);
		}
		...
		template<class X>
		this_type& operator= (const vector3t<X> &rhs){
...
#undef SC_
	...
	//@{
	template<class T>
	inline const vector3t<T> operator+ (const vector3t<T> & rhs){
		return rhs;
	}	
	template<class T>
	inline vector3t<T> operator- (const vector3t<T> & rhs){
...
#define DECLARE_OPERATOR(OP)												\
	template<class T>                                                       \
	inline vector3t<T> operator OP (const vector3t<T> &lhs, const vector3t<T> &rhs){	\
...
#undef DECLARE_OPERATOR
	 ...
	 */
	template<class T>
	inline T length(const vector3t<T> &rhs){
		return rhs.length();
	}
	/** 
	 *	@param rhs a vector.
	 ...
	 */
	template<class T>
	inline T sqr_length(const vector3t<T> &rhs){
	 ...
	 */
	template<class T>
	inline T sum(const vector3t<T> &rhs){
		return rhs.sum();
	}	
	 ...
	 */
	template<class T>
	inline vector3t<T> normalize(const vector3t<T> &rhs){
	...
	std::basic_ostream<CharT, Traits>& operator<<(std::basic_ostream<CharT, Traits>& os, const vector3t<T>& rhs){
